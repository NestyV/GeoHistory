...
const validatePlacePayload = (input: any, isPartial: boolean = false): Record<string, unknown> => {
  ...
  if (!isPartial || hasName) {
    ...
    if (typeof name !== 'string' || !name.trim()) {
      throw new ValidationError('Place current_name is required');
    }
  ...
  if (!isPartial || input?.place_type_id !== undefined) {
    if (typeof input?.place_type_id !== 'string' || !input.place_type_id.trim()) {
      throw new ValidationError('Place place_type_id is required');
    }
  ...
  if (!isPartial || hasLatitude) {
    ...
    if (latitude === undefined || latitude < -90 || latitude > 90) {
      throw new ValidationError('Place latitude must be between -90 and 90');
    }
    output.lat = latitude;
  }
  ...
  if (!isPartial || hasLongitude) {
    ...
    if (longitude === undefined || longitude < -180 || longitude > 180) {
      throw new ValidationError('Place longitude must be between -180 and 180');
    }
  ...
  if (input?.previous_name !== undefined) {
    if (typeof input.previous_name !== 'string') {
      throw new ValidationError('Place previous_name must be a string');
    }
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const { places, total } = await placeService.getAllPlaces(limit, offset);
...
router.get('/bounds', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (isNaN(minLatNum) || isNaN(maxLatNum) || isNaN(minLonNum) || isNaN(maxLonNum)) {
      throw new ValidationError('Bounds parameters must be valid numbers');
    }

    const places = await placeService.getPlacesByBounds(minLatNum, maxLatNum, minLonNum, maxLonNum);
...
router.get('/nearby', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const distanceKm = parseFloat((distance ?? radius) as string);

    if (isNaN(latitude) || isNaN(longitude) || isNaN(distanceKm)) {
      throw new ValidationError('Coordinates and distance must be valid numbers');
    }

    const places = await placeService.getNearbyPlaces(latitude, longitude, distanceKm);
...
router.post('/', authenticate, requireRole('curator', 'super_user'), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const created = await placeService.createPlace(payload as any, req.user!);
...
router.put('/:id', authenticate, requireRole('curator', 'super_user'), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const updated = await placeService.updatePlace(id, payload as any, req.user!);
...
router.delete('/:id', authenticate, requireRole('super_user'), async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    await placeService.deletePlace(id, req.user!);
