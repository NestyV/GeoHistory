...
const normalizeEventPayload = (input) => {
    ...
    if (latitude === undefined || latitude < -90 || latitude > 90) {
        throw new errors_1.ValidationError('Event latitude must be between -90 and 90');
    }
    if (longitude === undefined || longitude < -180 || longitude > 180) {
        throw new errors_1.ValidationError('Event longitude must be between -180 and 180');
    }
    return {
        title,
...
const normalizeEventUpdatePayload = (input) => {
    const output = {};
    if (input?.title !== undefined) {
        if (typeof input.title !== 'string' || !input.title.trim()) {
            throw new errors_1.ValidationError('Event title must be a non-empty string');
        }
        output.title = input.title.trim();
    }
    if (input?.description !== undefined) {
        if (typeof input.description !== 'string') {
            throw new errors_1.ValidationError('Event description must be a string');
        }
    ...
    if (input?.start_date !== undefined || input?.event_date !== undefined) {
        const startDate = typeof input.start_date === 'string'
            ...
            : (typeof input.event_date === 'string' ? input.event_date : '');
        if (!startDate) {
            throw new errors_1.ValidationError('Event start date must be a valid string');
        }
    ...
    if (input?.location !== undefined) {
        if (typeof input.location !== 'string') {
            throw new errors_1.ValidationError('Event location must be a string');
        }
        output.location = input.location;
    }
    if (input?.end_date !== undefined) {
        if (typeof input.end_date !== 'string') {
            throw new errors_1.ValidationError('Event end date must be a valid string');
        }
...
router.get('/', async (req, res, next) => {
    ...
    try {
        ...
        const { events, total } = await EventService_1.eventService.getAllApprovedEvents(limit, offset);
...
router.get('/my', auth_1.authenticate, async (req, res, next) => {
    const logger = req.logger;
    try {
        const events = await EventRepository_1.eventRepository.findByUserId(req.user.id);
...
router.get('/:id', async (req, res, next) => {
    ...
    try {
        ...
        if (event.status !== 'approved' && event.user_id !== req.user?.id && req.user?.role !== 'super_user') {
            throw new errors_1.ValidationError('Event not found');
        }
...
router.post('/', auth_1.authenticate, async (req, res, next) => {
    ...
    try {
        ...
        const event = await EventService_1.eventService.createEvent(data, req.user.id);
...
router.put('/:id', auth_1.authenticate, async (req, res, next) => {
    ...
    try {
        ...
        const event = await EventService_1.eventService.updateEvent(id, data, req.user);
...
router.delete('/:id', auth_1.authenticate, async (req, res, next) => {
    ...
    try {
        ...
        await EventService_1.eventService.deleteEvent(id, req.user);
 ...
 */
router.post('/:id/approve', auth_1.authenticate, (0, auth_1.requireRole)('curator', 'super_user'), async (req, res, next) => {
    const logger = req.logger;
    try {
        const { id } = req.params;
        if (!id) {
        ...
        const event = await EventService_1.eventService.approveEvent(id, req.user);
...
router.patch('/:id/approve', auth_1.authenticate, (0, auth_1.requireRole)('curator', 'super_user'), async (req, res, next) => {
    const logger = req.logger;
    try {
        const { id } = req.params;
        if (!id) {
 ...
 * Requires curator role
 */
router.post('/:id/reject', auth_1.authenticate, (0, auth_1.requireRole)('curator', 'super_user'), async (req, res, next) => {
    const logger = req.logger;
    try {
        const { id } = req.params;
        if (!id) {
        ...
        if (reason !== undefined && typeof reason !== 'string') {
            throw new errors_1.ValidationError('Reject reason must be a string');
        }
        const event = await EventService_1.eventService.rejectEvent(id, req.user, reason);
...
router.patch('/:id/reject', auth_1.authenticate, (0, auth_1.requireRole)('curator', 'super_user'), async (req, res, next) => {
    const logger = req.logger;
    try {
        const { id } = req.params;
        if (!id) {
